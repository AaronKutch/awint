...
//!
//! The macros from the `awint_macros` crate are reexported from the main
//! `awint` crate.
//!
//! ## Scope Dependencies
//!
...
//!   buffer to the sink components. When concatenations take the form `variable
//!   or constant with full range; var_1[..]; var_2[..]; var_3[..], ...`, the
//!   macros use `Bits::copy_` to directly copy without an intermediate buffer.
//!   This copy assigning mode cannot copy between `Bits` references that point
//!   to the same underlying storage, because it results in aliasing. Thus,
//!   trying to do something like `cc!(x; x)` results in the borrow checker
//!   complaining about macro generated variables within the macro being
//!   borrowed as both immutable and mutable. `cc!(x; x)` is semantically a
//!   no-op anyway, so it should not be used.
